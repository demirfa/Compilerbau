...
class SplError private constructor(val errorCode: Int, val position: Position, message: String) : RuntimeException(message) {
    ...
    companion object {
        ...
        fun AssignmentHasDifferentTypes(position: Position, left: Type, right: Type): SplError {
            return SplError(108, position, "assignment has different types: left = $left, right = $right")
        }

        fun AssignmentRequiresIntegers(position: Position, actual: Type): SplError {
            return SplError(109, position, "assignment requires integer variable: actual = $actual")
        }
        ...
        fun OperatorDifferentTypes(position: Position, left: Type, right: Type): SplError {
            return SplError(118, position, "expression combines different types: left = $left, right = $right")
        }

        fun ComparisonNonInteger(position: Position, actual: Type): SplError {
            return SplError(119, position, "comparison requires integer operands: actual = $actual")
        }

        fun ArithmeticOperatorNonInteger(position: Position, actual: Type): SplError {
            return SplError(120, position, "arithmetic operation requires integer operands: actual = $actual")
        }
