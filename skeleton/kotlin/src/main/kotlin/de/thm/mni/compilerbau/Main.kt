...
fun main(args: Array<String>) {    
    ...
    try {
        ...
        val table =
                TableBuilder(options.phaseOption == CommandLineOptions.PhaseOption.TABLES).buildSymbolTable(program)
        ...
        VarAllocator(
                options.phaseOption === CommandLineOptions.PhaseOption.VARS,
                options.ershovOptimization
        ).allocVars(program, table)
        if (options.phaseOption === CommandLineOptions.PhaseOption.VARS) exitProcess(0)

        try {
            options.outputWriter.use { out ->
                CodeGenerator(out, options.ershovOptimization).generateCode(
                        program,
                        table
                )
            }
